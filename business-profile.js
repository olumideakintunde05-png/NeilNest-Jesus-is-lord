...
function renderQuickContact() {
  ...
  if (b.website) items.push({ label: "Website", icon: '<circle cx="12" cy="12" r="9"/><path d="M3 12h18M12 3a15 15 0 0 1 0 18M12 3a15 15 0 0 0 0 18"/>', action: () => { if (!PROFILE.isOwner) trackAnalyticsEvent(PROFILE.businessId, "websiteClicks"); window.open(b.website.startsWith("http") ? b.website : "https://" + b.website, "_blank"); } });
...
function renderOwnerActions() {
  ...
  // Was selecting by exact href string `a[href="add-business.html"]`, which
  // breaks under Netlify's "Pretty URLs" asset optimization — it rewrites
  // that href to `/add-business` (no .html) in the deployed output, so the
  // exact-match selector finds nothing and .href on null throws, killing
  // the rest of this function. An id is immune to that rewrite.
  const editBusinessLink = document.getElementById("editBusinessLink");
  if (editBusinessLink) editBusinessLink.href = `add-business.html?businessId=${PROFILE.businessId}`;
